...
def visualize(items, bs, sample_steps, cfg_scale):
    ...
    for chunk in tqdm(list(get_chunks(items, bs)), unit='batch'):
        ...
        def padding_caption(caption, length):
            # cur_len = caption.attention_mask.sum(dim=1)
            # caption.input_ids[0][length-2:length] = caption.input_ids[0][cur_len-2:cur_len]
            caption.attention_mask[0][:length] = 1.0
            # caption.input_ids[0][cur_len-2:length-2] = 3
            return caption
        ...
        # caption_embs = torch.tensor(np.load('/home/jichuang/PixArt-sigma_custom/output/t5_feat.npz')['caption_embs']).to(device).to(weight_dtype)
        emb_masks = caption_token.attention_mask
        # print(emb_masks.sum(dim=1))
        caption_embs = caption_embs[:, None]
        # print(caption_embs.shape)
        null_y = null_caption_embs.repeat(len(prompts), 1, 1)[:, None]
        # null_y =  torch.tensor(np.load('/home/jichuang/PixArt-sigma/service/output/t5_feat.npz')['null_y']).to(device).to(weight_dtype)
        print(f'finish embedding')
        # np.savez(f'output/data/t5_feat_{chunk_id}.npz', emb_masks = emb_masks.cpu(), null_y = null_y.cpu(), caption_embs = caption_embs.cpu())
        # chunk_id = chunk_id+1
        # continue
        with torch.no_grad():
        ...
        # import numpy as np
        # np.savez("output/data/input_pixart.npz", samples = samples.cpu())
...
if __name__ == '__main__':
    ...
    max_sequence_length = {"alpha": 120, "sigma": 300}[args.version] # max_sequence_length = {"alpha": 120, "sigma": 300}[args.version]
    ...
    ## change shape
    # state_dict['state_dict']['y_embedder.y_embedding'] = state_dict['state_dict']['y_embedder.y_embedding'][:,:2048]
    # state_dict['state_dict']['y_embedder.y_proj.fc1.weight'] = state_dict['state_dict']['y_embedder.y_proj.fc1.weight'][:,:2048]
    ...
    tokenizer = T5Tokenizer.from_pretrained("/data/jichuang/pretrained_models/t5-v1_1-xl", subfolder="tokenizer")
    text_encoder = T5EncoderModel.from_pretrained("/data/jichuang/pretrained_models/t5-v1_1-xl", subfolder="text_encoder").to(device)
    ...
    work_dir = 'home/jichuang/PixArt-sigma_custom/output' # os.path.join(*args.model_path.split('/')[:-2])
