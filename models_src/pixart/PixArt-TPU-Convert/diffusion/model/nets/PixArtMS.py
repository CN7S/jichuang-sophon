...
class PixArtMSBlock(nn.Module):
    ...
    def forward_with_attn(self, x, y, t, HW=None, **kwargs):
        B, N, C = x.shape

        shift_msa, scale_msa, gate_msa, shift_mlp, scale_mlp, gate_mlp = (self.scale_shift_table[None] + t.reshape(B, 6, -1)).chunk(6, dim=1)
        x = x + self.drop_path(gate_msa * self.attn(t2i_modulate(self.norm1(x), shift_msa, scale_msa), HW=HW))
        x = x + self.cross_attn.forward_attn(x, y)
        x = x + self.drop_path(gate_mlp * self.mlp(t2i_modulate(self.norm2(x), shift_mlp, scale_mlp)))
...
class PixArtMS(PixArt):
    ...
    def __init__(
            ...
            pred_sigma=True,
            drop_path: float = 0.,
            caption_channels=4096, # 768, 4096
            pe_interpolation=1.,
            config=None,
    ...
    def forward(self, x, timestep, y, mask=None, data_info=None, **kwargs):
        ...
        """
        # np.savez("output/data/input_pixart.npz", x = x.cpu(), t = timestep.cpu(), y = y.cpu())
        # assert(0)
        # bs = x.shape[0]
        # x = x.to(self.dtype)
        ...
        # np.savez("output/data/output_embedding.npz", x = x.cpu(), t = t.cpu(), y = y.cpu(), mask = mask.cpu())
        # assert(0)
        # return x,t,y
        ...
        # ## get t_block input
        # np.savez('output/data/t_block_input.npz', t = t.cpu().detach().numpy())
        # ##

        # ## get dit_block input
        # np.savez('output/data/dit_block_input_with_t.npz', x = x.cpu().detach().numpy(), y = y.cpu().detach().numpy(), t = t.cpu().detach().numpy(), mask = mask.cpu().detach().numpy())
        # ##
        # assert(0)
        ...
        # ## get dit_block input
        # np.savez('output/data/dit_block_input.npz', x = x.cpu().detach().numpy(), y = y.cpu().detach().numpy(), t0 = t0.cpu().detach().numpy(), mask = mask.cpu().detach().numpy())
        # ##
        ...
        for block in self.blocks:
            ...
            self.h = self.w = 32
            x = block.forward_with_attn(x, y, t0, (self.h, self.w), **kwargs)
            # break


        # ## get final_layer input
        # np.savez('output/data/final_layer_input.npz', x = x.cpu().detach().numpy(), t = t.cpu().detach().numpy())
        # ##
        x = self.final_layer(x, t)  # (N, T, patch_size ** 2 * out_channels)
        x = self.unpatchify(x)  # (N, out_channels, H, W)
        # np.savez("output/data/output_pixart.npz", x = x.cpu())
        # assert(0)
        return x
    ...
    def forward_with_dpmsolver(self, x, timestep, y, data_info, **kwargs):
        ...
        # # save input
        # input('save input at forawrd func')
        # import numpy as np
        # np.savez("output/data/input_pixart.npz", x=x.cpu(), t=timestep.cpu(),y=y.cpu(), mask = kwargs['mask'].cpu())
        # print(x.shape, timestep.shape, y.shape)
        # ##
...
@MODELS.register_module()
def PixArtMS_L_2(**kwargs):
    return PixArtMS(depth=28, hidden_size=1152, patch_size=2, caption_channels=2048, num_heads=16, **kwargs)
