...
class MultiHeadCrossAttention(nn.Module):
    def __init__(self, d_model, num_heads, attn_drop=0., proj_drop=0., **block_kwargs):
        ...
        self.zeors_block = torch.zeros((1, 16, 1024, 50))

        import numpy as np
        self.attn_bias_const = torch.tensor(np.load('output/data/cross_attn_bias.npy'))
    ...
    def forward(self, x, cond, mask=None):
        ...
        def memory_efficient_attention_pytorch(query, key, value, attn_bias=None, p=0., scale=None):
            ...
            #     attn = attn + attn_bias
            # attn = attn.softmax(-1)
            len_H, len_W = attn.shape[-2:]
            len_H = len_H // 2
            len_W = len_W // 2
            # print(attn.shape)
            soft1 = attn[:,:,:len_H,:len_W].softmax(-1) # 1
            soft2 = attn[:,:,len_H:,len_W:].softmax(-1) # 4
            part1 = torch.cat([soft1, self.zeors_block], dim=-1) # 1 2
            part2 = torch.cat([self.zeors_block, soft2], dim=-1) # 3 4
            attn = torch.cat([part1, part2], dim=-2)
            # print(attn.shape)
        ...
        def diagonamask_from_seqlens(seqa, seqb):
            seqnum = len(seqa)
            seqa = [int(x) for x in seqa]
            seqb = [int(x) for x in seqb]
            seqlena = int(sum(seqa))  # 计算总长度并转换为整数
            seqlenb = int(sum(seqb))  # 计算总长度并转换为整数
            attn_bias = torch.zeros([1, self.num_heads, seqlena, seqlenb], dtype=q.dtype, device=q.device)
            attn_bias.fill_(float('-inf'))  # 填充负无穷大

            starta = 0
            startb = 0
            for i in range(seqnum):
                attn_bias[:,:,starta:starta+seqa[i], startb:startb+seqb[i]] = 0
                starta += seqa[i]
                startb += seqb[i]
        ...
        #     attn_bias = diagonamask_from_seqlens([N] * B, mask)

        # x = xformers.ops.memory_efficient_attention(q, k, v, p=self.attn_drop.p, attn_bias=attn_bias)
        
        # save attn_bias
        # import numpy as np
        # np.save('output/data/cross_attn_bias.npy', attn_bias.cpu().numpy())
        # input()
        # assert(0)

        # print(x)
        x = memory_efficient_attention_pytorch(q, k, v, p=self.attn_drop.p)
    ...
    def forward_attn(self, x, cond):
        ...
        def memory_efficient_attention_pytorch(query, key, value, attn_bias=None, p=0., scale=None):
            ...
            part2 = torch.cat([self.zeors_block, soft2], dim=-1) # 3 4
            attn = torch.cat([part1, part2], dim=-2)
            # attn[:,:,:len_H,:len_W] = attn[:,:,:len_H,:len_W].softmax(-1)
            # attn[:,:,len_H:,len_W:] = attn[:,:,len_H:,len_W:].softmax(-1)
            # attn[:,:,:len_H,len_W:] = 0
            # attn[:,:,len_H:,:len_W] = 0
            attn = F.dropout(attn, p)
...
class AttentionKVCompress(Attention_):
    ...
    def forward(self, x, mask=None, HW=None, block_id=None):
        ...
        #print(x.shape, B,N,C, mask)
        # x = xformers.ops.memory_efficient_attention(q, k, v, p=self.attn_drop.p, attn_bias=attn_bias)

        x = memory_efficient_attention_pytorch(q, k, v, p=self.attn_drop.p, attn_bias=attn_bias)
...
class CaptionEmbedder(nn.Module):
    ...
    def token_drop(self, caption, force_drop_ids=None):
        ...
        """
        if force_drop_ids is None:
            drop_ids =  torch.rand(caption.shape[0]) < self.uncond_prob # torch.rand(caption.shape[0]).cuda() < self.uncond_prob
        else:
            drop_ids = force_drop_ids == 1
