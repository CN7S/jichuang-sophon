...
def get_args():
    ...
    parser.add_argument('--txt_file', default='/home/jichuang/PixArt-sigma/textfile.txt', type=str)
    parser.add_argument('--model_path', default='output/pretrained_models/PixArt-XL-2-1024x1024.pth', type=str)
    parser.add_argument('--t5_path', default='output/pretrained_models/PixArt-XL-2-1024x1024.pth', type=str)
...
def visualize(items, bs, sample_steps, cfg_scale):
    ...
    for chunk in tqdm(list(get_chunks(items, bs)), unit='batch'):
        ...
        #      continue
            prompt_clean, _, hw, ar, custom_hw = prepare_prompt_ar(f'2D comic, {chunk[0]}', base_ratios, device=device, show=False)  # ar for aspect ratio
        ...
        caption_embs = caption_embs[:, None].to(device)
        null_y = null_caption_embs.repeat(len(prompts), 1, 1)[:, None].to(device)
        

        # def doublefeat(feat):
        #     #print(feat.shape)
        #     feat_2x = torch.zeros(1,1,feat.shape[-2], feat.shape[-1]*2)
        #     for i in range(0,feat.shape[-1]):
        #         feat_2x[:,:,:, i*2] = feat[:,:,:, i]
        #         feat_2x[:,:,:, i*2+1] = feat[:,:,:, i]
        #     return feat_2x.to(device)
        # caption_embs = doublefeat(caption_embs)
        # null_y = doublefeat(null_y)
        ...
        for i, sample in enumerate(samples):
            save_path = os.path.join(save_root, f"{chunk_id:08}.jpg")
...
if __name__ == '__main__':
    ...
    # if 'y_embedder.y_embedding' in state_dict['state_dict']:
    #     del state_dict['state_dict']['y_embedder.y_embedding']
    # if 'y_embedder.y_proj.fc1.weight' in state_dict['state_dict']:
    #     del state_dict['state_dict']['y_embedder.y_proj.fc1.weight']
    # print(state_dict['state_dict']['y_embedder.y_embedding'].shape)

    missing, unexpected = model.load_state_dict(state_dict['state_dict'], strict=False)
    print('Missing keys: ', missing)
    print('Unexpected keys', unexpected)

    # torch.save({"state_dict":model.state_dict()}, "/data/jichuang/pretrained_models/PixArt_512_CUS.pth")
    ...
    # tokenizer = T5Tokenizer.from_pretrained(args.t5_path, subfolder="tokenizer")
    # text_encoder = T5EncoderModel.from_pretrained(args.t5_path, subfolder="text_encoder").to(device)
    
    # weight = text_encoder.state_dict()
    # summem = 0
    # allmem = 0
    # for wname in weight:
    #     print(weight[wname].dtype)
    #     if(len(weight[wname].shape) == 2):
    #         allmem = allmem + weight[wname].shape[-1] * weight[wname].shape[-2]
    #     # def convert_to_int8(w):
    #     #     # print(w.shape, len(w.shape))
    #     #     if len(w.shape) == 2:
    #     #         maxweight = w.max(dim=1)[0]
    #     #         # print(maxweight.shape)
    #     #         for index, l in enumerate(w):
    #     #             intmax = 127/maxweight[index]
    #     #             w[index,:] = w[index,:]*intmax
    #     #         return w.to(torch.int8), maxweight
    #     #     return w, None
    #     # def convert_to_float(w, maxweight):
    #     #     w.to(weight_dtype)
    #     #     if len(w.shape) == 2:
    #     #         for index, l in enumerate(w):
    #     #             w[index,:] = w[index,:]/127.0*maxweight[index]
    #     #     return w
    #     # if(wname.find('.q.') != -1 
    #     #    or wname.find('.k.') != -1
    #     #    or wname.find('.v.') != -1
    #     #    or wname.find('.o.') != -1
    #     #    or wname.find('.wi_0.') != -1
    #     #    or wname.find('.wi_1.') != -1):
    #     #     print(wname)
    #     #     a,b=convert_to_int8(weight[wname])
    #     #     weight[wname]=convert_to_float(a,b)
    #     #     if(len(weight[wname].shape) == 2):
    #     #         summem = summem + weight[wname].shape[-1] * weight[wname].shape[-2]

    #     print(wname, weight[wname].shape)
    #     input()
    
    # text_encoder.load_state_dict(weight, strict=False)
    # print('sum : ', summem, 'all: ', allmem)
